...
int CMCtransaction::set_res_msg(int pos, int type, int writelen, ...)//書き込むレジスタのバイト数
{
	va_list arguments;

	//共通ヘッダ
	mcifmng.res_msg[pos].res0401.common_header.subheader[0] = 0x50;//サブヘッダ
	...
	if (type & (MC_TYP_READ_D | MC_TYP_READ_R | MC_TYP_WRITE_D | MC_TYP_WRITE_R)) {
		...
		//要求データ長	
		*((PUINT16)(mcifmng.res_msg[pos].res1401.data_len)) = 2 + 10 + writelen; //監視タイマ (2byte)+ 要求データ(10+writelen byte) 
